...
import { getFallbackRecommendations, getEmergencyFallback } from './fallback-recommendations';
    ...
    for (const modelName of AVAILABLE_MODELS) {
        ...
        while (attempts <= MAX_RETRIES) {
            try {
                ...
                if (isForbidden) {
                    console.error(`[Gemini] ❌ Access denied for ${modelName}. Check API key.`);
                    break; // Don't retry forbidden
                }
...
export async function generateRecommendations(
    ...
    try {
        ...
        console.warn('[Gemini] Recommendations failed, using fallback:', e);
        // Use smart fallback based on mood or time
        const fallbackQueries = getFallbackRecommendations({ mood, limit: 8 });
...
export async function generateSmartQueue(
    ...
    try {
        ...
        console.warn('[Gemini] Smart queue failed, using fallback:', e);
        // Fallback to artist-based and similar recommendations
        const fallbackQueries = [
            `${currentTrack.artist} top songs`,
            `Similar to ${currentTrack.title}`,
            'Mix playlist trending',
            'DJ mix popular songs',
            `Radio ${currentTrack.artist}`
        ].slice(0, queueLength);
        
        return fallbackQueries.map(query => ({
            query,
            reason: 'Up next in queue'
        }));
    }
}
...
export async function enhanceSearchQuery(query: string, context?: string): Promise<string> {
    ...
    try {
        ...
        console.warn('[Gemini] Query enhancement failed, using original query');
