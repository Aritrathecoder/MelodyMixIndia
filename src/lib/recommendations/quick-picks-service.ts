...
export class QuickPicksService {
    static async getQuickPicks(forceRefresh = false): Promise<QuickPicksResponse> {
        ...
        try {
            ...
            console.warn('[QuickPicks] API returned zero tracks.');
        } catch (e) {
            console.warn('[QuickPicks] Service error:', (e as Error).message);
        }

        // Fallback: return empty recommendations gracefully
        return {
            recommendations: [],
            metadata: { region: 'Unknown', timeOfDay: 'unknown', mood: 'Chill' },
        };
    }
