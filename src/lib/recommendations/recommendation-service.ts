...
export class RecommendationService {
    ...
    static async getPersonalizedMix(): Promise<{ tracks: Track[]; reason: string }> {
        try {
            ...
            console.warn('[RecService] API returned zero tracks.');
            
        } catch (error) {
            console.warn('[RecService] Failed to fetch mix:', (error as Error).message);
        }

        // Fallback: Return recently liked songs if everything fails or no tracks returned
