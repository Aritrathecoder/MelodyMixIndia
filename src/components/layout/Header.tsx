...
export function Header() {
    ...
    const { userId, email, displayName, avatarUrl, signOut, isGuest } = useAuthStore();
    ...
    const handleLogoutConfirm = async () => {
        ...
        // Reload for guest mode to fully clear state
        if (isGuest) {
                            ...
                            <DropdownMenu>
                                ...
                                >
                                    <div className="px-3 py-3 mb-1">
                                        <p className="text-sm font-semibold text-white">{displayName} {isGuest && '(Guest)'}</p>
                                        <p className="text-xs text-neutral-400 truncate mt-0.5">{email}</p>
                                        {isGuest && (
                                            <p className="text-xs text-orange-400 mt-1 flex items-center gap-1">
                                                <span>🎵</span>
                                                <span>Sign in to save your music</span>
                                            </p>
                                        )}
                                    </div>
                                    ...
                                    <DropdownMenuItem onClick={handleSignOut} className="gap-3 p-3 rounded-lg focus:bg-red-500/10 focus:text-red-400 text-red-400 cursor-pointer group">
